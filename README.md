...
The current global position is displayed using the *geographic coordinate system* GCS, a spherical coordinate system for measuring positions on Earth in terms of latitude $\phi$ and longitude $\lamda$, where $\phi$ is the angle between the equatorial plane and the position $P_x$ and $\lamda$ is the angle between a reference meridian and the position $P_x$, given in angular degrees *°N-°S* or *°W-°E*, respectively.
